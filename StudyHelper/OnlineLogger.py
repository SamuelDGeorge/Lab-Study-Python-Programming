...
def get_changed_cells(old_checkpoint, current_checkpoint):
    if len(old_checkpoint["cells"]) == len(current_checkpoint["cells"]):
        num_changes, cells_changed, new_contents = get_same_length_change(
...
def get_diff_dict(old_checkpoint, current_checkpoint, current_time):
    diff_dict = {}
    num_changes, cell_changed, change_type, new_content = get_changed_cells(
        old_checkpoint, current_checkpoint
    )
    diff_dict.update(
        {
...
def push_to_cloud(log):
    url = "https://us-east-1.aws.data.mongodb-api.com/app/rest-api-vsfoo/endpoint/add_log?db=studies&collection=python"
        ...
        if log.is_file():
            ...
            if should_update:
                with open(log, "w") as f:
                    ...
                    old["diffs"].append(
                        get_diff_dict(current_checkpoint, checkpoint, current_time)
                    )
                    old["current_checkpoint"].update(
                        {"time": current_time, "checkpoint": checkpoint}
                    )
...
def start(watch_file, course_id="NoCourseSpecified", base_id="", IRB_consent=True):
    if IRB_consent:
        ...
        print(
            "Please give consent to logging data by updating agreement variable to True"
        )
...
def compress_log(watch_file):
    ...
    if log.is_file():
        ...
        log_zip.write(
            log, filename_stem + "_log.json", compress_type=zipfile.ZIP_DEFLATED
        )
        log_zip.close()
        print(
            "Compressed log to: "
            + str(os.path.join(dir_path, filename_stem + ".compressed"))
        )
    else:
...
def compress_full_log(full_log_file, output_log_file):
    ...
    if len(old["checkpoints"]) > 0:
        ...
        compressed_log = {
            "original_checkpoint": {
                "time": oldest_checkpoint["time"],
                "checkpoint": parse_checkpoint(oldest_checkpoint["checkpoint"]),
            },
            "current_checkpoint": {
                "time": newest_checkpoint["time"],
                "checkpoint": parse_checkpoint(newest_checkpoint["checkpoint"]),
            },
            "diffs": [],
        }
        for i in range(0, len(old["checkpoints"]) - 1):
            current_time = old["checkpoints"][i + 1]["time"]
            compressed_log["diffs"].append(
                get_diff_dict(
                    parse_checkpoint(old["checkpoints"][i]["checkpoint"]),
                    parse_checkpoint(old["checkpoints"][i + 1]["checkpoint"]),
                    current_time,
                )
            )
    else:
...
def decompress_compressed_log(compressed_log, output_full_file):
    ...
    for i in range(0, len(compressed["diffs"])):
        ...
        new_cell_array = get_rebuilt_cells(
            starting_checkpoint["checkpoint"]["cells"], diff_info
        )
        checkpoint_dict = {}
